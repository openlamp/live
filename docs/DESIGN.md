...
- `openlamp-live` adds the **Ableton-native pack**: a MIDI track routed to the
  `LumiDeck` port carries **notes** (colour/power/animation triggers) and **CC
...
Config-as-code, from one source: **`ableton/mapping.spec.json`** mirrors the
LumiDeck convention (channels→groups, notes→actions, CC→params). From it:
...
- **the `.als` template** is authored once inside Live and exported (its binary,
  ...
  needs the tracks pre-named + pre-routed to `LumiDeck` on each group's channel.
