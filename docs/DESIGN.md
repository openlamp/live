...
- **Package the engine as a no-install desktop app** (`.app` / `.exe`, menu-bar,
  ...
  [openlamp/engine](https://github.com/openlamp/openlamp-engine-python).)
...
- Cost to the user: run the engine's
  [`midi.py`](https://github.com/openlamp/openlamp-engine-python/blob/main/midi.py), which opens the
...
Config-as-code, derived from one source: **`ableton/mapping.spec.json`** is
**generated** by `tools/sync_spec.py` from the
[openlamp-spec-midi](https://github.com/openlamp/openlamp-spec-midi) spec's own
`mapping.spec.json`, and carries the source version it was derived from. From it:
